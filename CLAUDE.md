...
- Cloudflare everything: Pages (front end), Workers or Pages Functions with Hono (API), D1 (database), R2 (files), KV (sessions and settings), Cron Triggers (digests), Resend (email).
- Front end: SvelteKit on Pages. Boring, standard dependencies. No Next.js, no Astro.
...
## Build order
1. Stage 1: scaffold Pages + Hono + D1 + KV, custom domain, Access OTP, Action Items module end to end. Done when Paul can log in by emailed PIN and CRUD action items persisting in D1.
...
## Deployment
- Git-connected Cloudflare Pages is the deploy model: commit, push, Cloudflare builds and deploys. Direct wrangler deploy only for quick throwaway tests.
...
- Secrets (ASANA_TOKEN, RESEND_API_KEY, AI keys) via `wrangler secret put` or Pages project settings. Never in code, never in wrangler.toml.
